...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>这里的j实际上就是<code>(i - 1) (i -2) ... (i - i) </code>的集合，因为不一定要连续，所以不一定要由前一个的状态推导而来。</p><h3 id="初始化-1" tabindex="-1"><a class="header-anchor" href="#初始化-1" aria-hidden="true">#</a> 初始化</h3><div class="language-c++ line-numbers-mode" data-ext="c++"><pre class="language-c++"><code>for(int i = 0; i &lt; nums.size(); i ++) {
    dp[i] = 1;
}
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>第一步和上一题相同，关键是第二步。</p><p>由于子序列的缘故，当<code>i - 1 != j - 1</code>时，我们不能单纯的认为当前的<code>dp[i][j]</code>就为0，因为<code>dp[i - 1][j - 1]</code>只是它的一个前导状态，它还有<code>dp[i][j -1]</code>和<code>dp[i -1][j]</code>这两个前导状态。</p><h3 id="初始化-3" tabindex="-1"><a class="header-anchor" href="#初始化-3" aria-hidden="true">#</a> 初始化</h3><p>当两个序列，任意一个序列的长度为0时，它们的最长重复子数组长度一定是零，我们将dp定义为全局变量，省去初始化。</p><h3 id="遍历-3" tabindex="-1"><a class="header-anchor" href="#遍历-3" aria-hidden="true">#</a> 遍历</h3><div class="language-c++ line-numbers-mode" data-ext="c++"><pre class="language-c++"><code>for(int i = 1; i &lt;= text1.size(); i ++) {
